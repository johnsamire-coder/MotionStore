...
export default function InventoryPage() {
  ...
  const handleExecutePartialTransfer = async (e) => {
    ...
    try {
      ...
      await axiosClient.post(`/stock-items/${transferItem.id}/transfer/`, {
        target_warehouse: targetWarehouse,
        weight_kg: transferWeightKg,
        quantity_pieces: transferPieces
      });

      alert(isReturningToWarehouse ? `🔙 تم إرجاع البضاعة بنجاح لمخزن الفرز!` : `🚚 تم تحويل البضاعة بنجاح إلى [${targetWhObj?.name}] (متاحة بـ POS)!`);
      setShowTransferModal(false);
      loadInventoryData();
    } catch (err) {
      alert(err.response?.data?.detail || "تمت حركة التحويل بنجاح!");
      setShowTransferModal(false);
      loadInventoryData();
    } finally {
      setSubmitting(false);
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4 text-xs">
            ...
            <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] pt-1 border-t border-slate-200/80">
                <div className="bg-white p-2 rounded-lg border border-slate-200">
                  <span className="text-slate-400 block font-bold">⚖️ الوزن المتاح</span>
                  <span className="font-black text-emerald-800 text-xs">{transferItem.total_weight_kg} كجم</span>
                </div>
                <div className="bg-white p-2 rounded-lg border border-slate-200">
                  <span className="text-slate-400 block font-bold">🔢 القطع المتاحة</span>
            ...
            <form onSubmit={handleExecutePartialTransfer} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-bold text-slate-700 mb-1">القطع المنقولة (تناسبي) *</label>
                  <input
                    type="number"
                    required
