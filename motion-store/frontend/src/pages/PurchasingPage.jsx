...
export default function PurchasingPage() {
  ...
  const handleSaveQuickProduct = async (e) => {
    if (e) e.preventDefault();
    if (!quickProdForm.name.trim() || !quickProdForm.category) return alert(t('purchasing.productName'));
    setSaving(true);
    try {
  ...
  const handleSaveInvoice = async (e) => {
    if (e) e.preventDefault();
    if (!invForm.supplier_id || !invForm.warehouse_id) return alert(t('purchasing.selectSupplier'));
    ...
    try {
      ...
      if (invMode === 'BALE') {
        ...
        const finalDesc = `${catName} - ${invForm.bale_content_details}`;
      ...
      setInvForm({
        ...invForm, bale_content_details: '', bale_price: '', bale_custom_weight: '', freight_cost: '0.00', stock_items: [], direct_items: []
      });
      fetchData();
      setTimeout(() => setSuccessMsg(''), 3000);
    } catch (err) {
  ...
  const addStockItem = () => setInvForm(prev => ({ ...prev, stock_items: [...prev.stock_items, { product_id: products[0]?.id || '', qty: 1, price: 0 }] }));
  const updateStockItem = (index, field, val) => { const newItems = [...invForm.stock_items]; newItems[index][field] = val; setInvForm(prev => ({ ...prev, stock_items: newItems })); };
  const removeStockItem = (index) => { const newItems = [...invForm.stock_items]; newItems.splice(index, 1); setInvForm(prev => ({ ...prev, stock_items: newItems })); };

  const addDirectItem = () => setInvForm(prev => ({ ...prev, direct_items: [...prev.direct_items, { description: '', qty: 1, price: 0 }] }));
  const updateDirectItem = (index, field, val) => { const newItems = [...invForm.direct_items]; newItems[index][field] = val; setInvForm(prev => ({ ...prev, direct_items: newItems })); };
  const removeDirectItem = (index) => { const newItems = [...invForm.direct_items]; newItems.splice(index, 1); setInvForm(prev => ({ ...prev, direct_items: newItems })); };

  const totalProcurement = invoices.reduce((sum, inv) => sum + parseFloat(inv.total_amount || 0), 0);
  const calculateInvoiceSubtotal = () => {
    if (invMode === 'BALE') return parseFloat(invForm.bale_price || 0);
    if (invMode === 'STOCK') return invForm.stock_items.reduce((sum, i) => sum + ((parseFloat(i.qty) || 0) * (parseFloat(i.price) || 0)), 0);
    if (invMode === 'DIRECT') return invForm.direct_items.reduce((sum, i) => sum + ((parseFloat(i.qty) || 0) * (parseFloat(i.price) || 0)), 0);
    return 0;
  };
  const currentInvoiceTotal = calculateInvoiceSubtotal() + parseFloat(invForm.freight_cost || 0);
    ...
    <div className="space-y-6 font-sans" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowSupModal(true)} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-bold transition flex items-center gap-1.5 cursor-pointer">
            <Users size={14}/> <span>{t('purchasing.newSupplier')}</span>
          </button>
          <button onClick={() => setShowInvModal(true)} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-black transition flex items-center gap-1.5 shadow cursor-pointer">
            <Plus size={14}/> <span>{t('purchasing.newInvoice')}</span>
          </button>
        </div>
      </div>
      ...
      {successMsg && (
        <div className="bg-emerald-50 border border-emerald-300 text-emerald-800 p-3 rounded-xl text-sm font-bold flex items-center gap-2">
          <CheckCircle2 className="text-emerald-600" size={18} /> <span>{successMsg}</span>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
          <form onSubmit={handleSaveInvoice} className="bg-white rounded-2xl max-w-2xl w-full p-6 space-y-5 shadow-2xl border">
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 w-full gap-1">
              <button type="button" onClick={() => setInvMode('BALE')} className={`flex-1 py-2 rounded-lg text-xs font-black transition flex justify-center items-center gap-1 cursor-pointer ${invMode === 'BALE' ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-200'}`}>
                <Package size={14}/> <span>{t('purchasing.baleMode')}</span>
              </button>
              <button type="button" onClick={() => setInvMode('STOCK')} className={`flex-1 py-2 rounded-lg text-xs font-black transition flex justify-center items-center gap-1 cursor-pointer ${invMode === 'STOCK' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-200'}`}>
                <ShoppingBag size={14}/> <span>{t('purchasing.stockMode')}</span>
              </button>
              <button type="button" onClick={() => setInvMode('DIRECT')} className={`flex-1 py-2 rounded-lg text-xs font-black transition flex justify-center items-center gap-1 cursor-pointer ${invMode === 'DIRECT' ? 'bg-purple-600 text-white shadow-md' : 'text-slate-600 hover:bg-slate-200'}`}>
                <ShoppingCart size={14}/> <span>{t('purchasing.directMode')}</span>
              </button>
            </div>
            ...
            {invMode === 'BALE' && (
              <div className="bg-emerald-50/50 p-4 rounded-xl border border-emerald-200 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-slate-600 mb-1">{isRTL ? 'تصنيف البالة (مثال: حريمي / رجالي)' : 'Bale Category'}</label>
                    <div className="flex gap-2">
                      <select value={invForm.bale_category_id} onChange={e => setInvForm({...invForm, bale_category_id: e.target.value})} className="flex-1 bg-white border border-slate-300 rounded-lg p-2.5 text-xs font-bold" required>
                        {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                      </select>
                      <button type="button" onClick={handleAddCategory} className="bg-slate-800 hover:bg-slate-700 text-white px-3 rounded-lg flex items-center justify-center transition cursor-pointer"><Plus size={16}/></button>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-600 mb-1">{isRTL ? 'تفاصيل / نوع (مثال: صيفي ميكس)' : 'Details / Type'}</label>
                    <input type="text" value={invForm.bale_content_details} onChange={e => setInvForm({...invForm, bale_content_details: e.target.value})} className="w-full bg-white border border-slate-300 rounded-lg p-2.5 text-xs font-bold" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input type="number" step="0.01" placeholder="0.00" value={invForm.bale_price} onChange={e => setInvForm({...invForm, bale_price: e.target.value})} className="w-full bg-white border border-emerald-300 rounded-lg p-2.5 text-sm font-black text-emerald-900" required />
                  </div>
                </div>
              </div>
            )}
            ...
            {invMode === 'STOCK' && (
              <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-200 space-y-3">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setShowQuickProdModal(true)} className="text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-2.5 py-1 rounded-lg font-black shadow cursor-pointer flex items-center gap-1"><Tag size={12}/> <span>{t('purchasing.quickCodeProduct')}</span></button>
                    <button type="button" onClick={addStockItem} className="text-xs bg-blue-600 hover:bg-blue-700 text-white px-2.5 py-1 rounded-lg font-black shadow cursor-pointer">{t('purchasing.addProduct')}</button>
                  </div>
                </div>
                {invForm.stock_items.length === 0 ? (
                  ...
                  invForm.stock_items.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 bg-white p-2 rounded-lg border shadow-sm">
                      <select value={item.product_id} onChange={e => updateStockItem(idx, 'product_id', e.target.value)} className="flex-1 bg-transparent border-0 outline-none text-xs font-bold text-slate-900">
                        {products.map(p => <option key={p.id} value={p.id}>{p.name} ({p.code || 'بدون كود'})</option>)}
                      </select>
                      <input type="number" placeholder={t('purchasing.qty')} value={item.qty} onChange={e => updateStockItem(idx, 'qty', e.target.value)} className="w-16 border rounded p-1 text-xs text-center font-bold" />
            ...
            {invMode === 'DIRECT' && (
              <div className="bg-purple-50/50 p-4 rounded-xl border border-purple-200 space-y-3">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button type="button" onClick={addDirectItem} className="text-xs bg-purple-600 hover:bg-purple-700 text-white px-3 py-1 rounded-lg font-black shadow cursor-pointer">{t('purchasing.addDirectItem')}</button>
                </div>
                {invForm.direct_items.length === 0 ? (
                  ...
                  invForm.direct_items.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-2 bg-white p-2 rounded-lg border shadow-sm">
                      <input type="text" placeholder={t('purchasing.freeTextItem')} value={item.description} onChange={e => updateDirectItem(idx, 'description', e.target.value)} className="flex-1 bg-transparent border-0 outline-none text-xs font-bold text-slate-900" />
                      <input type="number" placeholder={t('purchasing.qty')} value={item.qty} onChange={e => updateDirectItem(idx, 'qty', e.target.value)} className="w-16 border rounded p-1 text-xs text-center font-bold" />
                      <input type="number" step="0.01" placeholder={t('purchasing.unitPrice')} value={item.price} onChange={e => updateDirectItem(idx, 'price', e.target.value)} className="w-20 border rounded p-1 text-xs text-center font-bold text-purple-800" />
            ...
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex items-center justify-between gap-4">
              ...
              <input type="number" step="0.01" placeholder="0.00" value={invForm.freight_cost} onChange={e => setInvForm({...invForm, freight_cost: e.target.value})} className="w-32 bg-white border border-slate-300 rounded-lg p-2 text-xs font-black text-rose-600 text-center" />
            </div>
            ...
            <button type="submit" disabled={saving} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl font-black text-sm shadow transition cursor-pointer disabled:opacity-50">
          ...
          <form onSubmit={handleSaveQuickProduct} className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-emerald-300">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="text" placeholder={isRTL ? "مثال: تيشرت أوفر سايز زارا" : "e.g. Zara Oversize T-Shirt"} value={quickProdForm.name} onChange={e => setQuickProdForm({ ...quickProdForm, name: e.target.value })} className="w-full bg-slate-50 border border-slate-300 rounded-lg p-2.5 text-xs font-bold" required autoFocus />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">{t('purchasing.productCode')}</label>
                <input type="text" placeholder="SKU-1001" value={quickProdForm.code} onChange={e => setQuickProdForm({ ...quickProdForm, code: e.target.value })} className="w-full bg-slate-50 border border-slate-300 rounded-lg p-2 text-xs font-mono font-bold" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">{t('purchasing.category')}</label>
                <select value={quickProdForm.category} onChange={e => setQuickProdForm({ ...quickProdForm, category: e.target.value })} className="w-full bg-slate-50 border border-slate-300 rounded-lg p-2.5 text-xs font-bold" required>
                  {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">{t('purchasing.uom')}</label>
                <select value={quickProdForm.unit_of_measure} onChange={e => setQuickProdForm({ ...quickProdForm, unit_of_measure: e.target.value })} className="w-full bg-slate-50 border border-slate-300 rounded-lg p-2.5 text-xs font-bold">
            ...
            <button type="submit" disabled={saving} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl font-black text-sm shadow transition cursor-pointer">
